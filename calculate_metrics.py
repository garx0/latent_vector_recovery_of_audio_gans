...
inverse_mapping_model.load_weights("/imaging/akeyes/inverse_mapping_model/generated_and_real_training.ckpt")
...
def fid_score(audio, reconstruction):
  audio = tf.signal.stft(audio, 256, 128, pad_end=True)
  audio = tf.expand_dims(audio, -1)
  ...
  reconstruction = tf.signal.stft(reconstruction, 256, 128, pad_end=True)
  reconstruction = tf.expand_dims(reconstruction, -1)

  reconstruction = tf.stack([
    reconstruction,
    reconstruction,
    reconstruction
  ], axis=-1)
  ...
  if(len(tf.shape(reconstruction))<4):
      reconstruction = tf.expand_dims(reconstruction, 0)
  reconstruction = tf.cast(reconstruction, tf.float32)

  # training=False is only needed if there are layers with different
  # behavior during training versus inference (e.g. Dropout).
  audio_predictions = classifier(audio, training=False).numpy()
  rec_predictions = classifier(reconstruction, training=False).numpy()
  
  audio_predictions = np.asmatrix(audio_predictions)
  rec_predictions = np.asmatrix(rec_predictions)
  
  score = calculate_fid(audio_predictions, rec_predictions)
...
for idx, (label, audio) in enumerate(dataset):
    ...
    score = fid_score(reconstruction.numpy(), audio.numpy())
...
print("Inception Score (Mean): ", np.mean(scores))
print("Inception Score (Std): ", np.std(scores))
